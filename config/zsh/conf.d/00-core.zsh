...
# OS icon for prompt (requires Nerd Fonts patched font)
#
# Uses BMP PUA codepoints from the Nerd Fonts range. Without a patched
# font (e.g. MesloLGS Nerd Font) the icon renders as a missing-glyph box.
# printf with '\uXXXX' defers Unicode interpretation to runtime, unlike
# $'\uXXXX' which is expanded at parse time. 2>/dev/null || true ensures
